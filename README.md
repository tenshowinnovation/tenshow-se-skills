...
| [`volc-tos-storage-domain`](volc-tos-storage-domain/SKILL.md) | 火山引擎 TOS 对象存储自定义域名工作流：面向已备案/ICP-ready 的 `storage.<domain>` 公共资源域名，完成 TOS 绑定、DNS CNAME、DV 证书申请、HTTPS 证书绑定和对象 URL 验证。Volcengine TOS custom storage-domain workflow for ICP-ready public asset domains, covering bucket custom-domain binding, DNS CNAME, DV certificate issuance, HTTPS binding, and verification. |
